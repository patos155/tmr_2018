...
//Ultrasonicos 
     ...
     //Delantero izquierdo
       const int trigPin_DL = 28;
       const int echoPin_DL = 26;
      //Delantero derecho
       const int trigPin_DR = 22;
       const int echoPin_DR = 24;
     //Derecho
...
void setup() {
   ...
   // Sensor ultrasonico Delantero Izquierdo 
      pinMode(trigPin_DL, OUTPUT); 
      pinMode(echoPin_DL, INPUT); 
      digitalWrite(trigPin_DL, LOW);
   
   // Sensor ultrasonico Delantero Derecho
      pinMode(trigPin_DR, OUTPUT); 
      pinMode(echoPin_DR, INPUT); 
      digitalWrite(trigPin_DR, LOW);
...
void lee(){
    ...
    /*
    if (cm_D<d_fte && cm_DL<d_fte && cm_DR<d_fte) {
       adelante=0;
    }else {
       if ((cm_D<d_fte)) {
           adelante=2;
...
void muestra(){
    ...
    Serial.println(cm_DL);
    Serial.print("delantero Derecho  ");
    Serial.println(cm_DR);
...
/*
        //los sensores al frente se encuentran en 1-0-1 con obstaculo al frente (no pared)
        if (adelante == 2){
        ...
        //los sensores al frente se encuentran en 0-1-1 con obstaculo al frente (no pared)
        if (adelante == 3){
           // Obstaculo a la izquierda sin pared a la derecha
           if (izquierda==0 && derecha==1){
               giro_derecha_falso();
               avanza_falso();
               delay(1000);
           ...
           // Obstaculo a la izquierda sin pared a la izquierda
           if (izquierda==1 && derecha==0){
               giro_izquierda_falso();
               avanza_falso();
               delay(2000);
           ...
           //Choco contra la pared a la izquierda?
           if (izquierda==1 && derecha==1 && ul_giro=="I"){
               giro_derecha_falso();
               avanza_falso();
               delay(1000);
               giro_izquierda_falso();
        ...
        //los sensores al frente se encuentran en 1-1-0 con obstaculo al frente (no pared)
        if (adelante == 4){
           // Obstaculo a la derecha sin pared a la derecha
           if (izquierda==0 && derecha==1){
               giro_derecha_falso();
               avanza_falso();
               delay(2000);
           ...
           // Obstaculo a la derecha sin pared a la izquierda
           if (izquierda==1 && derecha==0){
               giro_izquierda_falso();
               avanza_falso();
               delay(1000);
           ...
           //Choco contra la pared a la derecha?
           if (izquierda==1 && derecha==1 && ul_giro=="D"){
               giro_izquierda_falso();
               avanza_falso();
               delay(1000);
               giro_derecha_falso();
...
void avanza(){
    ...
    if (dif_sf<(-3)){
        motori_D.setSpeed(ade_ordi+50); 
        motori_D.run(FORWARD);       
        motord_D.setSpeed(0);  
        motord_D.run(FORWARD);        
        delay(2000);
    } else {
        //DECIADO A LA DERECHA
        if (dif_sf>(3)){
            motori_D.setSpeed(0); 
            motori_D.run(FORWARD);       
            motord_D.setSpeed(ade_ordd-50);  
            motord_D.run(FORWARD);        
            delay(200);
        }else{
            motori_D.setSpeed(ade_ordi); 
...
void ultra_DL(){
  digitalWrite(trigPin_DL, HIGH);
  delayMicroseconds(5);
  digitalWrite(trigPin_DL, LOW);
  duration = pulseIn(echoPin_DL, HIGH); //Devuelve la longitud del pulso del pin Echo en us
  // Imprime valores por el puerto serie:
  // Convierte el tiempo de recepción del eco en distancia:
  inches = microsecondsToInches(duration);
  cm_DL = microsecondsToCentimeters(duration)+1;
}
// Sensor delantero
void ultra_DR(){
  digitalWrite(trigPin_DR, HIGH);
  delayMicroseconds(5);
  digitalWrite(trigPin_DR, LOW);
  duration = pulseIn(echoPin_DR, HIGH); //Devuelve la longitud del pulso del pin Echo en us
  // Imprime valores por el puerto serie:
  // Convierte el tiempo de recepción del eco en distancia:
  inches = microsecondsToInches(duration);
  cm_DR = microsecondsToCentimeters(duration)+1;
}
